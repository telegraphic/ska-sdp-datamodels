...
class TestRASCILAdvise(unittest.TestCase):
    ...
    def make_MS(self, nfreqwin=1, dopol=False, zerow=False):
        ...
        self.bvis_list = \
            [ingest_unittest_visibility(self.low,
                                        [self.frequency[i]],
                                        [self.channelwidth[i]],
                                        self.times,
                                        self.vis_pol,
        ...
        shutil.rmtree(rascil_path("test_results/test_rascil_advise.ms"), ignore_errors=True)
        if nfreqwin > 1:
            self.bvis_list = [concatenate_blockvisibility_frequency(self.bvis_list)]
            
        export_blockvisibility_to_ms(rascil_path("test_results/test_rascil_advise.ms"),
                                     self.bvis_list)
    ...
    def test_advise(self):
        ...
        numpy.testing.assert_almost_equal(advice['freq_sampling_primary_beam'], 2381946.1861413997)
        numpy.testing.assert_almost_equal(advice['nwpixels'], 18)
        numpy.testing.assert_almost_equal(advice['nwpixels_image'], 494)
        numpy.testing.assert_almost_equal(advice['time_sampling_primary_beam'], 2572.5018810327115)
