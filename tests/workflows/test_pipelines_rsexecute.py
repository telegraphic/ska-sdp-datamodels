...
class TestPipelineGraphs(unittest.TestCase):
    ...
    def actualSetUp(
        self, add_errors=False, nfreqwin=5, dospectral=True, dopol=False, zerow=True
    ):
        ...
        if add_errors:
            ...
            gt = simulate_gaintable(
                gt, phase_error=0.1, amplitude_error=0.0, smooth_channels=1, leakage=0.0
            )
            self.blockvis_list = [
                rsexecute.execute(apply_gaintable, nout=1)(self.blockvis_list[i], gt)
    ...
    def test_ical_skymodel_pipeline_exact(self):
        ...
        skymodel_list = [
            SkyModel(components=comp_list) for comp_list in self.components_list
        ]
        self.model_imagelist = rsexecute.scatter(self.model_imagelist)
        skymodel_list = rsexecute.scatter(skymodel_list)
    ...
    def test_ical_skymodel_pipeline_partial(self):
        ...
        skymodel_list = [
            SkyModel(components=comp_list) for comp_list in scaled_components_list
        ]
        self.model_imagelist = rsexecute.scatter(self.model_imagelist)
        skymodel_list = rsexecute.scatter(skymodel_list)
