...
class TestImagingDeconvolveGraph(unittest.TestCase):
    ...
    def actualSetUp(
        ...
        if self.persist:
            export_image_to_fits(
                model, "%s/test_imaging_serial_deconvolved_model.fits" % self.dir
            )
        if self.persist:
            export_image_to_fits(
                self.cmodel, "%s/test_imaging_serial_deconvolved_cmodel.fits" % self.dir
            )
    ...
    def test_deconvolve_spectral(self):
        ...
        dirty_imagelist = invert_list_serial_workflow(
            self.vis_list,
            self.model_imagelist,
            context="2d",
            dopsf=False,
        ...
        psf_imagelist = invert_list_serial_workflow(
            self.vis_list,
            self.model_imagelist,
            context="2d",
            dopsf=True,
    ...
    def test_deconvolve_and_restore_cube_mmclean(self):
        ...
        if self.persist:
            export_image_to_fits(
                restored, "%s/test_imaging_serial_mmclean_restored.fits" % (self.dir)
            )
    ...
    def test_deconvolve_and_restore_cube_mmclean_facets(self):
        ...
        if self.persist:
            export_image_to_fits(
                restored,
                "%s/test_imaging_serial_overlap_mmclean_restored.fits" % (self.dir),
            )
