...
class TestMIDSimulations(unittest.TestCase):
    ...
    def simulation(
        ...
        error_bvis_list = [
            sum_predict_results_rsexecute(
                predict_skymodel_list_rsexecute_workflow(
                    bvis, error_sm_list[ibvis], docal=True, context="2d"
                )
            )
        ...
        no_error_bvis_list = [
            sum_predict_results_rsexecute(
                predict_skymodel_list_rsexecute_workflow(
                    bvis, no_error_sm_list[ibvis], docal=True, context="2d"
                )
            )
    ...
    def get_args(self):
        ...
        parser.add_argument(
            "--integration_time", type=float, default=3600, help="Integration time (s)"
        )
        parser.add_argument(
            "--time_range",
            type=float,
    ...
    def test_wind(self):
        ...
        numpy.testing.assert_almost_equal(
            qa.data["max"], 0.00011024929534694913, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["min"], -0.00011024929534694913, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["rms"], 8.356611096276117e-06, 5, err_msg=str(qa)
        )
    ...
    def test_heterogeneous(self):
        ...
        numpy.testing.assert_almost_equal(
            qa.data["max"], 0.006260400046320458, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["min"], -0.00035819885879138, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["rms"], 3.728425607449823e-05, 5, err_msg=str(qa)
        )
    ...
    def test_random(self):
        ...
        numpy.testing.assert_almost_equal(
            qa.data["max"], 3.5821163782097796e-05, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["min"], -6.485629243186776e-05, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["rms"], 3.7233114793261304e-06, 5, err_msg=str(qa)
        )
    ...
    def test_polarisation(self):
        ...
        numpy.testing.assert_almost_equal(
            qa.data["max"], 0.0004365409779069447, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["min"], -0.00046135977866455243, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["rms"], 9.279652696332783e-06, 5, err_msg=str(qa)
        )
