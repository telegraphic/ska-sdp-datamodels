...
class TestMIDSimulations(unittest.TestCase):
    ...
    def simulation(
        ...
        bvis_list = create_standard_mid_simulation_rsexecute_workflow(
            band,
            rmax,
            phasecentre,
            time_range,
        ...
        advice_list = rsexecute.execute(advise_wide_field)(
            bvis_list[0], guard_band_image=1.0, delA=0.02, verbose=False
        )
        ...
        vp_list = [
            rsexecute.execute(create_vp)(vp, pbtype, pointingcentre=phasecentre)
            for vp in vp_list
        ]
        ...
        if mode == "random_pointing":
            ...
            (
                ...
                band, bvis_list, original_components, vp_directory=vp_directory
            )
        elif mode == "heterogeneous":
            # Different antennas
            no_error_gtl, error_gtl = create_heterogeneous_gaintable_rsexecute_workflow(
                band,
                bvis_list,
                original_components,
            ...
            # Polarised beams
            no_error_gtl, error_gtl = create_polarisation_gaintable_rsexecute_workflow(
                band,
                bvis_list,
                original_components,
                get_vp=partial(get_vp, vp_directory=""),
        ...
        error_sm_list = [
            [
                rsexecute.execute(SkyModel, nout=1)(
                    components=[original_components[i]], gaintable=error_gtl[ibv][i]
                )
        ...
        no_error_sm_list = [
            [
                rsexecute.execute(SkyModel, nout=1)(
                    components=[original_components[i]], gaintable=no_error_gtl[ibv][i]
                )
                for i, _ in enumerate(original_components)
            ]
            for ibv, bv in enumerate(bvis_list)
        ...
        error_bvis_list = [
            rsexecute.execute(copy_visibility)(bvis, zero=True) for bvis in bvis_list
        ]
        error_bvis_list = [
            sum_predict_results_rsexecute(
                predict_skymodel_list_rsexecute_workflow(
                    bvis, error_sm_list[ibvis], context="2d", docal=True
                )
            )
        ...
        no_error_bvis_list = [
            sum_predict_results_rsexecute(
                predict_skymodel_list_rsexecute_workflow(
                    bvis, no_error_sm_list[ibvis], context="2d", docal=True
                )
            )
        ...
        no_error_bvis = rsexecute.execute(concatenate_visibility, nout=1)(
            no_error_bvis_list
        )
        difference_bvis = subtract_list_rsexecute_workflow(
            [error_bvis], [no_error_bvis]
        )
        ...
        if self.persist:
            export_image_to_fits(
                error_dirty,
                "{}/test_mid_simulations_{}_dirty.fits".format(
                    rascil_path("test_results"), mode
                ),
            )
            export_blockvisibility_to_ms(
                "{}/test_mid_simulations_{}_difference.ms".format(
                    rascil_path("test_results"), mode
                ),
                bvis_list,
            )
    ...
    def get_args(self):
        ...
        parser.add_argument(
            "--integration_time", type=float, default=1800, help="Integration time (s)"
        )
        parser.add_argument(
            "--time_range",
            type=float,
        ...
        parser.add_argument(
            "--flux_limit", type=float, default=0.01, help="Flux limit (Jy)"
        )
        ...
        parser.add_argument(
            "--shared_directory",
            type=str,
            default=rascil_data_path("configurations"),
        ...
        parser.add_argument(
            "--height", type=float, default=3e5, help="Height of layer (meters)"
        )
        parser.add_argument(
            "--screen",
            type=str,
        ...
        parser.add_argument(
            "--time_chunk", type=float, default=3600.0, help="Time for a chunk (s)"
        )
        parser.add_argument(
            "--mode",
            type=str,
        ...
        parser.add_argument(
            "--vp_directory",
            type=str,
            default=rascil_data_path("models/interpolated"),
            help="Location of voltage pattern files",
        )
        parser.add_argument(
            "--show", type=str, default="False", help="Show details of simulation?"
        )
    ...
    def test_wind(self):
        ...
        error_dirty, sumwt = self.simulation(
            args,
            "wind_pointing",
            image_polarisation_frame=PolarisationFrame("stokesIQUV"),
            vis_polarisation_frame=PolarisationFrame("linear"),
        ...
        numpy.testing.assert_almost_equal(
            qa.data["max"], 0.000139353747921011, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["min"], -0.0028688981199968574, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["rms"], 1.6083641925025978e-05, 5, err_msg=str(qa)
        )
    ...
    def test_heterogeneous(self):
        ...
        numpy.testing.assert_almost_equal(
            qa.data["max"], 0.007721348155708392, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["min"], -0.0006678172286967584, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["rms"], 5.212090558783337e-05, 5, err_msg=str(qa)
        )
    ...
    def test_random(self):
        ...
        numpy.testing.assert_almost_equal(
            qa.data["max"], 2.472368365158522e-06, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["min"], -9.762213174928972e-07, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["rms"], 7.831098142486326e-08, 5, err_msg=str(qa)
        )
    ...
    def test_surface(self):
        ...
        if os.path.isdir(rascil_path("models/interpolated")):
            ...
            numpy.testing.assert_almost_equal(
                qa.data["max"], 2.2055849698035616e-06, 5, err_msg=str(qa)
            )
            numpy.testing.assert_almost_equal(
                qa.data["min"], -6.838117387793031e-07, 5, err_msg=str(qa)
            )
            numpy.testing.assert_almost_equal(
                qa.data["rms"], 3.7224203394509413e-07, 5, err_msg=str(qa)
            )
        else:
            log.warning(
                "test_surface: Interpolated voltage patterns not available - cannot test surface"
            )
    ...
    def test_polarisation(self):
        ...
        numpy.testing.assert_almost_equal(
            qa.data["max"], 0.00037863455250564473, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["min"], -0.0004809777038127784, 5, err_msg=str(qa)
        )
        numpy.testing.assert_almost_equal(
            qa.data["rms"], 1.2845605938834913e-05, 5, err_msg=str(qa)
        )
