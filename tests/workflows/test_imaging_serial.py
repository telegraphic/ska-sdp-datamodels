...
class TestImaging(unittest.TestCase):
    ...
    def actualSetUp(
        ...
        self.model_list = [
            insert_skycomponent(self.model_list[freqwin], self.components_list[freqwin])
            for freqwin, _ in enumerate(self.frequency)
        ]
            ...
            self.gcfcf_clipped = [
                (
                    ...
                    apply_bounding_box_convolutionfunction(
                        self.gcfcf[0][1], fractional_level=1e-3
                    ),
                )
            ]
    ...
    def _predict_base(
        ...
        vis_list = predict_list_serial_workflow(
            vis_list,
            self.model_list,
            context=context,
        ...
        dirty = invert_list_serial_workflow(
            vis_list,
            self.model_list,
            context=context,
            dopsf=False,
        ...
        if self.persist:
            export_image_to_fits(
                dirty[0],
                "%s/test_imaging_predict_%s%s_serial_dirty.fits"
    ...
    @unittest.skip(
        "Timeslice predict needs better interpolation and facets need overlap"
    )
    def test_predict_facets_timeslice(self):
        self.actualSetUp()
        self._predict_base(
            context="facets_timeslice",
            fluxthreshold=19.0,
            facets=8,
    ...
    def test_predict_facets_wstack(self):
        self.actualSetUp()
        self._predict_base(
            context="facets_wstack", fluxthreshold=15.0, facets=8, vis_slices=101
        )
    ...
    def test_predict_timeslice(self):
        self.actualSetUp()
        self._predict_base(
            context="timeslice", fluxthreshold=5.5, vis_slices=self.ntimes
        )
    ...
    def test_predict_wstack_spectral(self):
        ...
        self._predict_base(
            context="wstack", extra="_spectral", fluxthreshold=4.0, vis_slices=51
        )
    ...
    def test_invert_facets(self):
        self.actualSetUp()
        self._invert_base(
            context="facets", positionthreshold=2.0, check_components=True, facets=8
        )
    ...
    def test_invert_wstack_spectral(self):
        self.actualSetUp(dospectral=True, block=False)
        self._invert_base(
            context="wstack", extra="_spectral", positionthreshold=2.0, vis_slices=51
        )
    ...
    def test_zero_list(self):
        ...
        assert numpy.max(numpy.abs(predicted_vis_list[centre].vis)) > 0.0, numpy.max(
            numpy.abs(predicted_vis_list[centre].vis)
        )

        diff_vis_list = subtract_list_serial_workflow(
            self.bvis_list, predicted_vis_list
        )

        assert numpy.max(numpy.abs(diff_vis_list[centre].vis)) < 1e-15, numpy.max(
    ...
    def test_residual_list(self):
        ...
        centre = self.freqwin // 2
        residual_image_list = residual_list_serial_workflow(
    ...
    def test_restored_list(self):
        ...
        centre = self.freqwin // 2
        psf_image_list = invert_list_serial_workflow(
            self.bvis_list, self.model_list, context="2d", dopsf=True
        )
        residual_image_list = residual_list_serial_workflow(
            self.bvis_list, self.model_list, context="2d"
        )
        restored_image_list = restore_list_serial_workflow(
        ...
        if self.persist:
            export_image_to_fits(
                ...
                "%s/test_imaging_invert_serial_restored.fits" % (self.dir),
            )
    ...
    def test_restored_list_noresidual(self):
        ...
        if self.persist:
            export_image_to_fits(
                restored_image_list[centre],
                "%s/test_imaging_invert_serial_restored_noresidual.fits" % (self.dir),
            )
    ...
    def test_restored_list_facet(self):
        ...
        if self.persist:
            export_image_to_fits(
                ...
                "%s/test_imaging_invert_serial_restored_4facets.fits" % (self.dir),
            )
