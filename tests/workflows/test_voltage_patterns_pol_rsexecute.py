...
class TestVoltagePatternsPolGraph(unittest.TestCase):
    ...
    def _test(self, time_range=None, flux=None, test_vp=False, name=""):
        ...
        for method in ["fft", "dft"]:
            ...
            if method == "dft":
                ...
                vp_list = [
                    rsexecute.execute(make_ejterm_rotated)(centre_model[ibvis], bvis)
                    for ibvis, bvis in enumerate(bvis_graph)
                ]
                vp_list = rsexecute.persist(vp_list)

                dirty_list = calculate_residual_fft_rsexecute_workflow(
                    bvis_graph,
                    original_components,
                    future_model_list,
                    vp_list=vp_list,
            ...
            for ipol, pol in enumerate(["I", "Q", "U", "V"]):
                ...
                result["peak_{}_{}".format(method, pol)] = max(
                    qa.data["min"], qa.data["max"], key=abs
                )
        ...
        for ipol, pol in enumerate(["I", "Q", "U", "V"]):
            result["peak_diff_{}".format(pol)] = (
                result["peak_fft_{}".format(pol)] - result["peak_dft_{}".format(pol)]
            )
            result["peak_modeldiff_{}".format(pol)] = (
                result["peak_dft_{}".format(pol)] - result["model_{}".format(pol)]
            )
            if self.verbose:
                print(
                    "{} model: {:.2f} fft: {:.6f} dft: {:.6f} fft - dft: {:.6f} dft - model: {:.6f}".format(
                        pol,
                        result["model_{}".format(pol)],
    ...
    def check_values(self, result, tolerance=1e-3):
        for ipol, pol in enumerate(["I", "Q", "U", "V"]):
            ...
            assert (
                abs(error) < tolerance
            ), "Stokes {}, error in peak too large: {}".format(pol, error)
