...
class TestSkyModel(unittest.TestCase):
    ...
    def test_predict(self):
        ...
        assert isinstance(self.skymodel_list[0].image, Image), self.skymodel_list[
            0
        ].image
        assert isinstance(
            self.skymodel_list[0].components[0], Skycomponent
        ), self.skymodel_list[0].components[0]
        assert len(self.skymodel_list[0].components) == 26, len(
            self.skymodel_list[0].components
        )
        assert (
            numpy.max(numpy.abs(self.skymodel_list[0].image.data)) > 0.0
        ), "Image is empty"
