...
class TestBufferDataModelHelpers(unittest.TestCase):
    ...
    def test_readwriteskymodel(self):
        ...
        assert (
            numpy.max(numpy.abs(newsm.image["pixels"].data - im["pixels"].data)) < 1e-15
        )
    ...
    def test_readwriteconvolutionfunction(self):
        ...
        bdm = BufferConvolutionFunction(
            config["buffer"], config["convolutionfunction"], cf
        )
        bdm.sync()
        new_bdm = BufferConvolutionFunction(
            config["buffer"], config["convolutionfunction"]
        )
        new_bdm.sync()
