...
class TestDataModelHelpers(unittest.TestCase):
    ...
    def test_readwritevisibility(self):
        ...
        export_visibility_to_hdf5(
            self.vis, "%s/test_data_model_helpers_visibility.hdf" % self.dir
        )
        newvis = import_visibility_from_hdf5(
            "%s/test_data_model_helpers_visibility.hdf" % self.dir
        )

        assert str(newvis) == str(self.vis), "Original %s, import %s" % (
            str(newvis),
            str(self.vis),
        )
        ...
        assert (
            numpy.abs(
                newvis.configuration.location.x.value
                - self.vis.configuration.location.x.value
            )
            < 1e-15
        )
        assert (
            numpy.abs(
                newvis.configuration.location.y.value
                - self.vis.configuration.location.y.value
            )
            < 1e-15
        )
        assert (
            numpy.abs(
                newvis.configuration.location.z.value
                - self.vis.configuration.location.z.value
            )
            < 1e-15
        )
        assert (
            numpy.max(numpy.abs(newvis.configuration.xyz - self.vis.configuration.xyz))
            < 1e-15
        )
    ...
    def test_readwritepointingtable(self):
        ...
        export_pointingtable_to_hdf5(
            gt, "%s/test_data_model_helpers_pointingtable.hdf" % self.dir
        )
        newgt = import_pointingtable_from_hdf5(
