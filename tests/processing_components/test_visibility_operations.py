...
class TestVisibilityOperations(unittest.TestCase):
    ...
    def test_create_visibility_from_rows1(self):
        ...
        for makecopy in [True, False]:
            selected_vis = create_visibility_from_rows(
                self.vis, rows, makecopy=makecopy
            )
    ...
    def test_convert_blockvisibility(self):
        ...
        assert (
            numpy.unique(vis.time).size == self.vis.time.size
    ...
    def test_phase_rotation_inverse(self):
        ...
        original_uvw = self.vismodel.uvw
        rotatedvis = phaserotate_visibility(
            phaserotate_visibility(self.vismodel, there, tangent=False, inverse=True),
            self.phasecentre,
            tangent=False,
            inverse=True,
        )
