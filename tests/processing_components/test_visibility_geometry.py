...
class TestGeometry(unittest.TestCase):
    ...
    def test_azel(self):
        ...
        numpy.testing.assert_array_almost_equal(azel[0][0].deg, 152.555168)
        numpy.testing.assert_array_almost_equal(azel[1][0].deg, 24.025615)

    def test_hourangle(self):
        ha = calculate_blockvisibility_hourangles(self.bvis)
        numpy.testing.assert_array_almost_equal(ha[0].deg, -89.999775)

    def test_parallatic_angle(self):
        pa = calculate_blockvisibility_parallactic_angles(self.bvis)
        numpy.testing.assert_array_almost_equal(pa[0].deg, -101.938617)

    def test_transit_time(self):
        transit_time = calculate_blockvisibility_transit_time(self.bvis)
        numpy.testing.assert_array_almost_equal(transit_time.mjd, 58849.89581)
