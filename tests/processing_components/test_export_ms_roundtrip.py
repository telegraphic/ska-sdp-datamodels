...
class measurementset_tests(unittest.TestCase):
    ...
    def test_roundtrip(self):
        ...
        m31image = create_test_image(
            frequency=frequency, cellsize=cellsize, phasecentre=vt.phasecentre
        )
        ...
        error = numpy.max(numpy.abs(dirty_after.data - dirty_before.data)) / numpy.max(
            numpy.abs(dirty_after.data)
        )
