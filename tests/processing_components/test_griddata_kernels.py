""" Unit tests for gridding kernels
...
class TestGridDataKernels(unittest.TestCase):
    ...
    def test_fill_box_to_convolutionfunction(self):
        gcf, cf = create_box_convolutionfunction(
            self.image, polarisation_frame=PolarisationFrame("linear")
        )
        assert gcf["pixels"].data.dtype == "float", gcf["pixels"].data.dtype
        assert numpy.max(numpy.abs(cf["pixels"].data)) > 0.0
        if self.persist:
            export_image_to_fits(
                gcf, "%s/test_convolutionfunction_box_gcf.fits" % self.results_dir
            )
            export_convolutionfunction_to_fits(
                cf, "%s/test_convolutionfunction_box_cf.fits" % self.results_dir
            )
        ...
        assert (
            numpy.abs(cf["pixels"].data[peak_location] - 1.0) < 1e-15
    ...
    def test_fill_pswf_to_convolutionfunction(self):
        ...
        if self.persist:
            export_image_to_fits(
                gcf, "%s/test_convolutionfunction_pswf_gcf.fits" % self.results_dir
            )
            export_convolutionfunction_to_fits(
                cf, "%s/test_convolutionfunction_pswf_cf.fits" % self.results_dir
            )
        ...
        assert (
            numpy.abs(cf["pixels"].data[peak_location] - 0.18712109669890534 + 0j)
            < 1e-7
    ...
    def test_fill_wterm_to_convolutionfunction(self):
        ...
        assert (
            numpy.abs(
                cf_clipped["pixels"].data[peak_location] - (0.1870600903328245 - 0j)
            )
            < 1e-7
    ...
    def test_fill_aterm_to_convolutionfunction(self):
        ...
        assert (
            numpy.abs(cf["pixels"].data[peak_location] - 0.07761522554113436 - 0j)
            < 1e-7
    ...
    def test_compare_wterm_symmetry(self):
        ...
        assert (
            numpy.abs(cf["pixels"].data[peak_location] - (0.18704542575148253 - 0j))
            < 1e-12
        ), cf["pixels"].data[peak_location]
        ...
        assert (
            numpy.abs(cf["pixels"].data[p1] - numpy.conjugate(cf["pixels"].data[p2]))
            < 1e-15
        )
