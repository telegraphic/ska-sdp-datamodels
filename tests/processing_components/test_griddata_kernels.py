...
from rascil.processing_components import create_image, fft_image, create_w_term_like
...
class TestGridDataKernels(unittest.TestCase):
    ...
    def test_fill_box_to_convolutionfunction(self):
        ...
        assert (
            numpy.abs(cf.data[peak_location] - 1.0) < 1e-15
        ), "Peak is incorrect %s" % str(cf.data[peak_location] - 1.0)
    ...
    def test_fill_pswf_to_convolutionfunction(self):
        ...
        if self.persist:
            export_image_to_fits(
                cf_image, "%s/test_convolutionfunction_pwsf_cf.fits" % self.dir
            )

        peak_location = numpy.unravel_index(numpy.argmax(numpy.abs(cf.data)), cf.shape)
        assert (
            numpy.abs(cf.data[peak_location] - 0.18712109669890534 + 0j) < 1e-7
    ...
    def test_fill_pswf_to_convolutionfunction_nooversampling(self):
        ...
        if self.persist:
            export_image_to_fits(
                gcf,
                "%s/test_convolutionfunction_pswf_nooversampling_gcf.fits" % self.dir,
            )
        ...
        if self.persist:
            export_image_to_fits(
                cf_image,
                "%s/test_convolutionfunction_pwsf_nooversampling_cf.fits" % self.dir,
            )

        peak_location = numpy.unravel_index(numpy.argmax(numpy.abs(cf.data)), cf.shape)
        assert (
            numpy.abs(cf.data[peak_location] - 0.18712109669890536 + 0j) < 1e-7
        ), cf.data[peak_location]
    ...
    def test_fill_wterm_to_convolutionfunction(self):
        ...
        if self.persist:
            export_image_to_fits(
                cf_image, "%s/test_convolutionfunction_wterm_cf.fits" % self.dir
            )

        peak_location = numpy.unravel_index(numpy.argmax(numpy.abs(cf.data)), cf.shape)
        assert peak_location == (0, 0, 100, 4, 4, 30, 30), peak_location
        assert (
            numpy.abs(cf.data[peak_location] - (0.1870600903328245 - 0j)) < 1e-7
        ), cf.data[peak_location]
        u_peak, v_peak = cf.grid_wcs.sub([1, 2]).wcs_pix2world(
            peak_location[-2], peak_location[-1], 0
        )
        ...
        if self.persist:
            export_image_to_fits(
                cf_image,
                "%s/test_convolutionfunction_wterm_clipped_real_cf.fits" % self.dir,
            )
        cf_image = convert_convolutionfunction_to_image(cf_clipped)
        ...
        if self.persist:
            export_image_to_fits(
                cf_image,
                "%s/test_convolutionfunction_wterm_clipped_imag_cf.fits" % self.dir,
            )
        ...
        assert (
            numpy.abs(cf_clipped.data[peak_location] - (0.1870600903328245 - 0j)) < 1e-7
    ...
    def test_fill_wterm_to_convolutionfunction_nopswf(self):
        ...
        if self.persist:
            export_image_to_fits(
                gcf, "%s/test_convolutionfunction_wterm_nopswf_gcf.fits" % self.dir
            )
        ...
        if self.persist:
            export_image_to_fits(
                cf_image, "%s/test_convolutionfunction_wterm_nopswf_cf.fits" % self.dir
            )
    ...
    def test_fill_awterm_to_convolutionfunction(self):
        ...
        if self.persist:
            export_image_to_fits(
                cf_image, "%s/test_convolutionfunction_awterm_cf.fits" % self.dir
            )

        peak_location = numpy.unravel_index(numpy.argmax(numpy.abs(cf.data)), cf.shape)
        assert peak_location == (0, 0, 100, 4, 4, 30, 30), peak_location
        assert (
            numpy.abs(cf.data[peak_location] - (0.07761529943522588 - 0j)) < 1e-7
        ), cf.data[peak_location]
        u_peak, v_peak = cf.grid_wcs.sub([1, 2]).wcs_pix2world(
            peak_location[-2], peak_location[-1], 0
        )
        ...
        if self.persist:
            export_image_to_fits(
                cf_image,
                "%s/test_convolutionfunction_awterm_clipped_cf.fits" % self.dir,
            )
    ...
    def test_fill_aterm_to_convolutionfunction(self):
        ...
        if self.persist:
            export_image_to_fits(
                cf_image, "%s/test_convolutionfunction_aterm_cf.fits" % self.dir
            )

        peak_location = numpy.unravel_index(numpy.argmax(numpy.abs(cf.data)), cf.shape)
        assert (
            numpy.abs(cf.data[peak_location] - 0.07761522554113436 - 0j) < 1e-7
        ), cf.data[peak_location]
        ...
        if self.persist:
            export_image_to_fits(
                cf_image, "%s/test_convolutionfunction_aterm_clipped_cf.fits" % self.dir
            )
        peak_location = numpy.unravel_index(
            numpy.argmax(numpy.abs(cf_clipped.data)), cf_clipped.shape
        )
    ...
    def test_compare_aterm_kernels(self):
        ...
        _, cf = create_awterm_convolutionfunction(
            self.image, make_pb=make_pb, oversampling=16, support=32, use_aaf=True
        )
        cf.data = numpy.real(cf.data)
        _, cf_noover = create_awterm_convolutionfunction(
            self.image, make_pb=make_pb, oversampling=1, support=32, use_aaf=True
        )
        cf_noover.data = numpy.real(cf_noover.data)
        cf.data[...] -= cf_noover.data[0, 0, 0, 0, 0]
        cf.data[...] /= numpy.max(cf_noover.data)
        assert numpy.abs(cf.data[0, 0, 0, 8, 8, 16, 16]) < 1e-6, cf.data[
            0, 0, 0, 8, 8, 16, 16
        ]

    def test_compare_wterm_kernels(self):
        _, cf = create_awterm_convolutionfunction(
            self.image, nw=110, wstep=8, oversampling=8, support=60, use_aaf=True
        )
        cf.data = numpy.real(cf.data)
        ...
        assert (
            numpy.abs(cf.data[peak_location] - (0.18704542575148253 - 0j)) < 1e-7
        ), cf.data[peak_location]

        _, cf_noover = create_awterm_convolutionfunction(
            self.image, nw=111, wstep=8, oversampling=1, support=60, use_aaf=True
        )
        cf_noover.data = numpy.real(cf_noover.data)
        cf.data[...] -= cf_noover.data[0, 0, 0, 0, 0]
        ...
        if self.persist:
            export_image_to_fits(
                cf_image,
                "%s/test_convolutionfunction_compare_wterm_kernels.fits" % self.dir,
            )

        assert numpy.abs(cf.data[0, 0, 0, 4, 4, 30, 30]) < 5e-6, cf.data[
            0, 0, 0, 4, 4, 30, 30
        ]
    ...
    def test_compare_wterm_symmetry(self):
        ...
        assert peak_location == (0, 0, 55, 4, 4, 30, 30), peak_location
        assert (
            numpy.abs(cf.data[peak_location] - (0.18704542575148253 - 0j)) < 1e-12
        ), cf.data[peak_location]
    ...
    def test_fill_aterm_to_convolutionfunction_noover(self):
        ...
        if self.persist:
            export_image_to_fits(
                cf_image, "%s/test_convolutionfunction_aterm_noover_cf.fits" % self.dir
            )

        peak_location = numpy.unravel_index(numpy.argmax(numpy.abs(cf.data)), cf.shape)
        assert (
            numpy.abs(cf.data[peak_location] - 0.0776153022780847 + 0j) < 1e-7
        ), cf.data[peak_location]
        ...
        if self.persist:
            export_image_to_fits(
                gcf, "%s/test_convolutionfunction_aterm_noover_gcf.fits" % self.dir
            )
        ...
        if self.persist:
            export_image_to_fits(
                cf_image,
                "%s/test_convolutionfunction_aterm_clipped_noover_cf.fits" % self.dir,
            )
        peak_location = numpy.unravel_index(
            numpy.argmax(numpy.abs(cf_clipped.data)), cf_clipped.shape
        )
