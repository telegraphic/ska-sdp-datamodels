...
class TestImage(unittest.TestCase):
    ...
    def test_create_image_from_array(self):
        m31model_by_array = create_image_from_array(
            self.m31image.data, self.m31image.wcs, self.m31image.polarisation_frame
        )
        add_image(self.m31image, m31model_by_array)
        assert m31model_by_array.shape == self.m31image.shape
        if self.persist:
            export_image_to_fits(
                self.m31image, fitsfile="%s/test_model.fits" % (self.dir)
            )
    ...
    def test_calculate_image_frequency_moments(self):
        ...
        cube = create_low_test_image_from_gleam(
            npixel=512, cellsize=0.0001, frequency=frequency, flux_limit=1.0
        )
        if self.persist:
            export_image_to_fits(
                cube, fitsfile="%s/test_moments_cube.fits" % (self.dir)
            )
    ...
    def test_calculate_image_frequency_moments_1(self):
        ...
        if self.persist:
            export_image_to_fits(
                cube, fitsfile="%s/test_moments_1_cube.fits" % (self.dir)
            )
        original_cube = copy_image(cube)
    ...
    def test_fftim(self):
        self.m31image = create_test_image(
            cellsize=0.001, frequency=[1e8], canonical=True
        )
        m31_fft = fft_image(self.m31image)
        m31_fft_ifft = fft_image(m31_fft, self.m31image)
        numpy.testing.assert_array_almost_equal(
            self.m31image.data, m31_fft_ifft.data.real, 12
        )
    ...
    def test_fftim_factors(self):
        for i in [3, 5, 7]:
            ...
            numpy.testing.assert_array_almost_equal(
                padded.data, padded_fft_ifft.data.real, 12
            )
    ...
    def test_apply_voltage_pattern(self):
        ...
        m31image = create_test_image(
            cellsize=cellsize, frequency=[1.36e9], canonical=True
        )
