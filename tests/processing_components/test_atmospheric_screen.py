...
from rascil.processing_components.simulation.atmospheric_screen import (
...
class TestAtmosphericScreen(unittest.TestCase):
    ...
    def test_create_gaintable_from_screen_ionosphere(self):
        ...
        beam = create_test_image(
            cellsize=0.0015, frequency=self.frequency, phasecentre=self.vis.phasecentre
        )
        ...
        gaintables = create_gaintable_from_screen(
            self.vis, actual_components, screen, height=3e5
        )
    ...
    def test_create_gaintable_from_screen_troposphere(self):
        ...
        screen = rascil_data_path("models/test_mpc_screen.fits")
        beam = create_test_image(
            cellsize=0.00015, frequency=self.frequency, phasecentre=self.vis.phasecentre
        )
        ...
        actual_components = filter_skycomponents_by_flux(
            pb_s3_components, flux_max=10.0
        )
    ...
    def test_grid_gaintable_to_screen(self):
        ...
        old_screen = import_xarray_from_fits(
            rascil_data_path("models/test_mpc_screen.fits")
        )
        ...
        if self.persist:
            export_image_to_fits(
                newscreen, rascil_path("test_results/test_mpc_screen_gridded.fits")
            )
        if self.persist:
            export_image_to_fits(
                weights,
    ...
    def test_plot_gaintable_to_screen(self):
        ...
        plot_gaintable_on_screen(
            self.vis,
            gaintables,
            plotfile=rascil_path("test_results/test_plot_gaintable_to_screen.png"),
        )
