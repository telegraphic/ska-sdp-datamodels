...
class TestImaging2D(unittest.TestCase):
    ...
    def test_invert_2d(self):
        self.actualSetUp(zerow=True)
        self._invert_base(
            name="invert_2d", positionthreshold=2.0, check_components=True
        )
    ...
    def test_invert_2d_block(self):
        self.actualSetUp(zerow=True, block=True)
        self._invert_base(
            name="invert_2d_block", positionthreshold=2.0, check_components=True
        )
    ...
    def test_predict_awterm_spec_IQUV(self):
        ...
        self._predict_base(
            fluxthreshold=35.0, name="predict_awterm_spec_IQUV", gcfcf=gcfcf
        )
    ...
    def test_predict_wterm(self):
        self.actualSetUp(zerow=False)
        gcf, cf = create_awterm_convolutionfunction(
            self.model, nw=50, wstep=16.0, oversampling=4, support=100, use_aaf=True
        )
        cf_clipped = apply_bounding_box_convolutionfunction(cf, 1e-4)
        gcfcf = (gcf, cf_clipped)
    ...
    def test_invert_psf_block(self):
        ...
        if self.persist:
            export_image_to_fits(
                psf[0], "%s/test_imaging_2d_psf_block.fits" % (self.dir)
            )
    ...
    def test_invert_psf_weighting_block(self):
        ...
        for weighting in ["natural", "uniform", "robust"]:
            self.vis = weight_visibility(
                self.vis, self.model, weighting=weighting, robustness=-1.0
            )
            psf = invert_2d(self.vis, self.model, dopsf=True)
            error = numpy.max(psf[0].data) - 1.0
            assert abs(error) < 1.0e-12, error
            if self.persist:
                export_image_to_fits(
                    psf[0],
                    "%s/test_imaging_2d_psf_block_%s.fits" % (self.dir, weighting),
                )
            assert numpy.max(numpy.abs(psf[0].data)), "Image is empty"

    def test_invert_psf_weighting_block_IQUV(self):
        self.actualSetUp(
            zerow=False, block=True, image_pol=PolarisationFrame("stokesIQUV")
        )
        for weighting in ["natural", "uniform", "robust"]:
            self.vis = weight_visibility(
                self.vis, self.model, weighting=weighting, robustness=-1.0
            ...
            if self.persist:
                export_image_to_fits(
                    psf[0],
                    "%s/test_imaging_2d_psf_block_%s_IQUV.fits" % (self.dir, weighting),
                )
            assert numpy.max(numpy.abs(psf[0].data)), "Image is empty"
