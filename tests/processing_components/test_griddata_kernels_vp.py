...
class TestGridDataKernels(unittest.TestCase):
    ...
    def test_fill_vpterm_to_convolutionfunction(self):
        ...
        gcf, cf = create_vpterm_convolutionfunction(
            self.image, make_vp=make_vp, oversampling=16, support=32, use_aaf=True
        )
        cf_image = convert_convolutionfunction_to_image(cf)
        cf_image.data = numpy.real(cf_image.data)
        if self.persist:
            export_image_to_fits(
                cf_image, "%s/test_convolutionfunction_aterm_vp_cf.fits" % self.dir
            )
        ...
        assert (
            numpy.abs(
                cf.data[peak_location] - (0.005285675638650622 + 0.000494340010248879j)
            )
            < 1e-7
