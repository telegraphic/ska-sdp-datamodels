...
class TestCoalesce(unittest.TestCase):
    def setUp(self):
        ...
        self.phasecentre = SkyCoord(
            ra=+0.0 * u.deg, dec=-35.0 * u.deg, frame="icrs", equinox="J2000"
        )
    ...
    def test_coalesce_decoalesce_frequency(self):
        cvis = coalesce_visibility(
            self.blockvis, time_coal=0.0, max_time_coal=1, frequency_coal=1.0
        )
        assert numpy.min(cvis.frequency) == numpy.min(self.frequency)
        assert numpy.min(cvis.frequency) > 0.0
        dvis = decoalesce_visibility(cvis)
    ...
    def test_coalesce_decoalesce_time(self):
        cvis = coalesce_visibility(
            self.blockvis, time_coal=1.0, frequency_coal=0.0, max_frequency_coal=1
        )
        assert numpy.min(cvis.frequency) == numpy.min(self.frequency)
        assert numpy.min(cvis.frequency) > 0.0
        dvis = decoalesce_visibility(cvis)
