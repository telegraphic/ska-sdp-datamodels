...
class TestImageDeconvolutionMSMFSPol(unittest.TestCase):
    ...
    def test_deconvolve_mmclean_no_taylor_pol_nowindow(self):
        ...
        if self.persist:
            export_image_to_fits(
                self.comp,
                "%s/test_deconvolve_mmclean_notaylor_nowindow-comp.fits" % self.dir,
            )
        if self.persist:
            export_image_to_fits(
                self.residual,
    ...
    def test_deconvolve_mmclean_no_taylor_pol(self):
        ...
        if self.persist:
            export_image_to_fits(
                self.comp, "%s/test_deconvolve_mmclean_notaylor-comp.fits" % self.dir
            )
        if self.persist:
            export_image_to_fits(
                self.residual,
    ...
    def test_deconvolve_mmclean_no_taylor_noscales_nowindow_pol(self):
        ...
        if self.persist:
            export_image_to_fits(
                self.comp,
                "%s/test_deconvolve_mmclean_notaylor_noscales_nowindow-comp.fits"
                % self.dir,
            )
        if self.persist:
            export_image_to_fits(
    ...
    def test_deconvolve_mmclean_quadratic_pol(self):
        ...
        if self.persist:
            export_image_to_fits(
                self.comp, "%s/test_deconvolve_mmclean_quadratic-comp.fits" % self.dir
            )
        if self.persist:
            export_image_to_fits(
                self.residual,
