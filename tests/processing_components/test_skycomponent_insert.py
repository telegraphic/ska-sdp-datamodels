...
class TestSkyComponentInsert(unittest.TestCase):
    ...
    def test_insert_skycomponent_FFT_IQUV(self):
        ...
        assert_array_almost_equal(
            self.model["pixels"].data[2, :, rpix[1], rpix[0]], self.flux[3, :], 8
        )
    ...
    def test_insert_skycomponent_nearest_IQUV(self):
        ...
        for pol in range(4):
            self.assertAlmostEqual(
                self.model["pixels"].data[2, pol, 151, 122], self.pol_flux[pol], 7
            )
    ...
    def test_insert_skycomponent_sinc(self):
        ...
        # These test a regression but are not known a priori to be correct
        self.assertAlmostEqual(
            self.model["pixels"].data[2, 0, 151, 122], 0.87684398703184396, 7
        )
        self.assertAlmostEqual(
            self.model["pixels"].data[2, 0, 152, 122], 0.2469311811046056, 7
        )
    ...
    def test_insert_skycomponent_sinc_bandwidth(self):
        ...
        # These test a regression but are not known a priori to be correct
        self.assertAlmostEqual(
            self.model["pixels"].data[2, 0, 151, 122], 0.25133066186805758, 7
        )
        self.assertAlmostEqual(
            self.model["pixels"].data[2, 0, 152, 122], 0.19685222464041874, 7
        )
    ...
    def test_insert_skycomponent_lanczos(self):
        ...
        # These test a regression but are not known a priori to be correct
        self.assertAlmostEqual(
            self.model["pixels"].data[2, 0, 151, 122], 0.87781267543090036, 7
        )
        self.assertAlmostEqual(
            self.model["pixels"].data[2, 0, 152, 122], 0.23817562762032077, 7
        )
    ...
    def test_insert_skycomponent_lanczos_IQUV(self):
        ...
        for pol in range(4):
            self.assertAlmostEqual(
                self.model["pixels"].data[2, pol, 151, 122],
                self.pol_flux[pol] * 0.87781267543090036,
                7,
            )
            self.assertAlmostEqual(
                self.model["pixels"].data[2, pol, 152, 122],
                self.pol_flux[pol] * 0.23817562762032077,
                7,
            )
    ...
    def test_insert_skycomponent_lanczos_bandwidth(self):
        ...
        # These test a regression but are not known a priori to be correct
        self.assertAlmostEqual(
            self.model["pixels"].data[2, 0, 151, 122], 0.24031092091707615, 7
        )
        self.assertAlmostEqual(
            self.model["pixels"].data[2, 0, 152, 122], 0.18648989466050975, 7
        )
