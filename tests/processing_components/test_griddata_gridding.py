...
class TestGridDataGridding(unittest.TestCase):
    ...
    def actualSetUp(
        self, zerow=True, block=False, image_pol=PolarisationFrame("stokesIQUV")
    ):
        self.doplot = False
        self.npixel = 256
    ...
    def test_griddata_invert_pswf_block(self):
        ...
        if self.persist:
            export_image_to_fits(
                im, "%s/test_gridding_dirty_pswf_block.fits" % self.dir
            )
        self.check_peaks(im, 97.10594988489598, tol=1e-7)
    ...
    def test_griddata_invert_aterm_noover(self):
        ...
        if self.persist:
            export_image_to_fits(pb, "%s/test_gridding_aterm_pb.fits" % self.dir)
        gcf, cf = create_awterm_convolutionfunction(
            self.model, make_pb=make_pb, nw=1, oversampling=1, support=16, use_aaf=True
        )
        griddata = create_griddata_from_image(self.model, self.vis)
        griddata, sumwt = grid_visibility_to_griddata(
            self.vis, griddata=griddata, cf=cf
    ...
    def test_griddata_invert_wterm(self):
        self.actualSetUp(zerow=False)
        gcf, cf = create_awterm_convolutionfunction(
            self.model, nw=100, wstep=8.0, oversampling=4, support=32, use_aaf=True
        )
    ...
    def test_griddata_invert_awterm(self):
        ...
        self.check_peaks(im, 97.1325877407122)
    ...
    def test_griddata_predict_pswf(self):
        ...
        gcf, cf = create_pswf_convolutionfunction(
            self.model, support=8, oversampling=255
        )
    ...
    def test_griddata_predict_aterm(self):
        ...
        griddata = create_griddata_from_image(modelIQUV, self.vis)
        gcf, cf = create_awterm_convolutionfunction(
            modelIQUV, make_pb=make_pb, nw=1, oversampling=16, support=32, use_aaf=True
        )
        griddata = fft_image_to_griddata(modelIQUV, griddata, gcf)
        newvis = degrid_visibility_from_griddata(self.vis, griddata=griddata, cf=cf)
        qa = qa_visibility(newvis)
    ...
    def test_griddata_predict_wterm(self):
        ...
        gcf, cf = create_awterm_convolutionfunction(
            self.model, nw=100, wstep=10.0, oversampling=16, support=32, use_aaf=True
        )
        modelIQUV = convert_stokes_to_polimage(self.model, self.vis.polarisation_frame)
        griddata = create_griddata_from_image(modelIQUV, self.vis)
        griddata = fft_image_to_griddata(modelIQUV, griddata, gcf)
    ...
    def test_griddata_visibility_weight(self):
        ...
        self.check_peaks(im, 99.40822097132116)
    ...
    def test_griddata_blockvisibility_weight(self):
        self.actualSetUp(
            zerow=True, block=True, image_pol=PolarisationFrame("stokesIQUV")
        )
        gcf, cf = create_pswf_convolutionfunction(self.model)
        gd = create_griddata_from_image(self.model, self.vis)
        gd_list = [
        ...
        self.check_peaks(im, 99.40822097131749)
    ...
    def test_griddata_blockvisibility_weight_I(self):
        ...
        if self.persist:
            export_image_to_fits(
                im, "%s/test_gridding_dirty_2d_IQ_uniform_block.fits" % self.dir
            )
        self.check_peaks(im, 99.4082209713175)

    def test_griddata_blockvisibility_weight_IQ(self):
        self.actualSetUp(
            zerow=True, block=True, image_pol=PolarisationFrame("stokesIQ")
        )
        gcf, cf = create_pswf_convolutionfunction(self.model)
        gd = create_griddata_from_image(self.model, self.vis)
        gd_list = [
